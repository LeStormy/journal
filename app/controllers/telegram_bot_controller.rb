...
class TelegramBotController < ApplicationController
  ...
  def generate_word_cloud
    ...
    if entries.present?
      ...
      sorted_words = word_freq.sort_by { |word, count| -count }.first(15)

      # Format the response as a text-based word cloud
      response = "Top 15 most common words for #{start_date.strftime('%B %Y')}:\n\n"
  ...
  def stopwords
    ...
    [
      'people', 'don', 'guess', 'day', 'feel', 'yea', 'gonna', 'kinda', 'wanna', 'lot', 'point', 'dont', 'ive', "ill", "lil", "theyre"
    ]
  end
end
